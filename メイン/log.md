...
## [2026-08-04] setup | Discordログのローカル取得基盤を追加

- Discord Botで閲覧可能な全チャンネル・スレッドを列挙し、初回全件・以後差分でJSONL保存するスクリプトを追加
- 添付ファイル取得、レート制限再試行、チェックポイント、取得レポートを実装
- 顧客情報を含み得る `メイン/raw/discord/` はGit管理外とし、wikiへは個人情報を除いた蒸留知識だけを反映する規約を追加

## [2026-08-04] ingest | Discord業務ログ 2022〜2026

- Botが閲覧できた通常チャンネル122件とスレッド37件、計33,623メッセージを対象に蒸留
- 案件名を含む82チャンネルを本文分析から除外し、顧客名・連絡先・認証情報・私的会話をwikiへ転記しない条件で処理
- source 1件、concept 1件を追加し、Yamasemi・原価管理パイプライン・注文住宅の業務プロセスを更新
- 既製SaaSからYamasemiへ移った経緯、動詞軸からフェーズ軸へのタスク設計転換、ツール選定の反復パターンを統合
...
## [2026-08-04] ingest | Yamasemi Discord履歴検索設計

- Yamasemi Issue #24、Draft PR #25、設計書から、[[Discord履歴検索]]の責任分界、データモデル、認可、AI送信ゲート、同期限界を蒸留
- [[Yamasemi]]の`/chat`記述を旧状態として残し、画面とAPIは撤去済み、社内チャットはDBのみ温存という2026-08-04時点の状態を追記
- 顧客ログ原文、チャンネル名、投稿者名、認証情報はWikiへ同期せず、再利用できる設計判断だけを記録
