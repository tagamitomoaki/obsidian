...
- [[Discord履歴検索]]（専用の読取ミラー、チャンネル単位で分離した検索許可と外部AI送信許可、Discord原文リンク、任意の根拠付きAI回答）はコード実装済み。archive再取得・dry-run・Message Content Intent・本番DB read-only preflight・role timeout確認は完了。本番migration、実取込、適用後の実RLS検証は未実施
